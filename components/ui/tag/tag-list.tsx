...
export const TagList: React.FC<TagListProps> = ({ tags, customTagRenderer, direction, draggable, onSortEnd, disabled, ...tagListProps }) => {
  ...
  return (
    ...
    >
      {draggable ? (
        <SortableList onSortEnd={onSortEnd} className="flex flex-wrap gap-2 list" dropTarget={<DropTarget />}>
          {tags.map((tagObj) => (
            <SortableItem key={tagObj.id}>
              ...
              >
                {customTagRenderer ? customTagRenderer(tagObj) : <Tag tagObj={tagObj} {...tagListProps} disabled={disabled} />}
              </div>
            </SortableItem>
          ))}
        </SortableList>
        ...
        tags.map((tagObj) =>
          customTagRenderer ? customTagRenderer(tagObj) : <Tag key={tagObj.id} tagObj={tagObj} {...tagListProps} disabled={disabled} />
        )
      )}
    </div>
  );
