...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>((props, _ref) => {
  ...
  if ((maxTags !== undefined && maxTags < 0) || (props.minTags !== undefined && props.minTags < 0)) {
    ...
    toast.error('maxTags and minTags cannot be less than 0|Please set maxTags and minTags to a value greater than or equal to 0');
    return null;
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (delimiterList ? delimiterList.includes(e.key) : e.key === delimiter || e.key === Delimiter.Enter || e.key === Delimiter.tab) {
      e.preventDefault();
  ...
  return (
    <div className={`w-full flex ${inputFieldPostion === 'bottom' ? 'flex-col' : inputFieldPostion === 'top' ? 'flex-col-reverse' : 'flex-row'}`}>
      {!usePopoverForTags ? (
        <TagList
          tags={truncatedTags}
      ...
      {enableAutocomplete ? (
        <div className="w-full max-w-[450px]">
          ...
          >
            {!usePopoverForTags ? (
              <CommandInput
                placeholder={(maxTags !== undefined && tags.length >= maxTags) || disabled ? placeholderWhenFull : placeholder}
                ref={inputRef}
                value={inputValue}
